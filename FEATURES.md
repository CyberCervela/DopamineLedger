...
| **Export / Import** | `done` | Full backup/restore. Settings → Danger Zone → Export (share sheet, JSON) / Import (file picker, replace-all). Round-trip unit test enforces field sync. `#if DEBUG` Load Test Data button. `Services/DataExporter.swift`. Session 39. |
