...
| **SessionView — credits as hero** | Reorder the center block so the credit number (not the timer) is the 64pt focal point. Spec: (1) credits value at 64pt, same color logic (green charger / red overrun / textPrimary normal spender); (2) "credits earned" / "credits spent" caption directly below; (3) elapsed timer (e.g. "07:45") at bodyStrong where the credit label currently sits; (4) supporting row (remaining / debt / burn-down / rate) unchanged. Live Activity intentionally excluded — timer auto-ticks for free there; credits are a pushed value and would look stale. In-app only. Files: `SessionView.swift` only. Effort: ~10 lines reordered. |
