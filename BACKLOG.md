...
| **Linked app catalog — research top apps to add** | Current catalog: YouTube, Twitter/X, Instagram, Snapchat, Facebook, TikTok, Netflix, Chrome. Research the top downloaded apps in the App Store (Entertainment, Social, Lifestyle categories) and decide which to add as both linked app catalog entries (LinkedApp.swift) and as app-specific templates (ActivityTemplate.swift). Candidates to investigate: Amazon Prime Video, Disney+, Apple TV+, Twitch, Reddit, Spotify, BeReal, Threads, WhatsApp, Telegram, X (separate from Twitter?), Pinterest, LinkedIn. Also consider charger-side apps: Kindle, Duolingo, Apple Fitness+, Headspace/Calm. Research session before any code — confirm URL schemes are stable, not just guessed. |
