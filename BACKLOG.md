...
| **Linked app catalog — expand with researched apps** | Research done (Session 32). Priority additions listed below. All URL schemes must be verified with `canOpenURL` on a real device before shipping — schemes can change between app versions. **Entertainment/streaming (spender):** Disney+ `disneyplus://`, Amazon Prime Video `aiv://`, Max (HBO) `max://`, Hulu `hulu://`, Twitch `twitch://`, Paramount+ `cbsallaccess://`. **Social (spender, high-risk):** Threads `threads://`, Reddit `reddit://`, Pinterest `pinterest://`, LinkedIn `linkedin://`, BeReal `bereal://` (verify). **Messaging (ambiguous — deep convo = charger, group chat = spender):** WhatsApp `whatsapp://`, Telegram `telegram://`. Leave unlinked by default; let user decide. **Gaming:** Most top games (Roblox excluded) don't expose URL schemes — generic Gaming template is the right approach; no individual game links planned. Roblox `roblox://` is an exception worth adding. **Charger-side apps (positive use case — link to good habits):** Duolingo `duolingo://`, Kindle `kindle://`, Headspace `headspace://`, Calm `calm://`, Notion `notion://`, Forest `forest://`. These would be new CHARGER templates with linked apps — the gatekeeper also works as an intentional entry point into productive apps, not just consumption ones. **Spotify:** Borderline — music while working = charger behaviour, but passive listening is a spender. Leave as user choice, no template. |
