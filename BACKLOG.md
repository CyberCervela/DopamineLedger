...
| ~~**Linked app catalog — expand with researched apps**~~ | ✅ Done Session 34. Catalog grew from 8 → 19 apps. Templates grew from 19 → 29. **Still needs device verification:** Disney+ (`disneyplus://`), Prime Video (`aiv://`), Max (`max://`), Threads (`threads://`), Reddit (`reddit://`), Roblox (`roblox://`), BeReal (`bereal://`) — tap each in a running session; any that trigger the "not installed" alert incorrectly need to be removed or replaced. **Paramount+ skipped:** `cbsallaccess://` is the pre-2021 CBS All Access scheme and is almost certainly dead. Community sources show only HTTPS universal links for Paramount+ now. Test `paramountplus://` on device before adding. **Netflix watch:** `nflx://` confirmed in cataloguing repos but a tvOS breakage was reported Sept 2025; iOS status unclear — test on device. **Not added (messaging):** WhatsApp, Telegram — ambiguous charger/spender use; left as user choice. **Not added (charger-side):** Duolingo, Kindle, Headspace, Calm, Notion, Forest — confirmed schemes exist; add as charger templates in a dedicated charger-side session. |
