...
| **History tab — bundle repeated short sessions — needs design** | When the same activity is run many times in a day (e.g. 6 × 3-min Doomscrolling sessions), the History tab shows a long list of near-identical rows that obscures the bigger picture. Idea: collapse runs of the same activity within a day into a single bundled row ("Doomscrolling · 6 sessions · 18 min total · 144 cr spent"). Open design questions for the ideation session: What's the threshold — same activity within the same day? Same hour? Only when ≥ N sessions? Should the row be expandable to show individual entries? What does the bundled icon/layout look like vs. a single-session row? Does this affect quest rows too (probably not — quests are one-off by nature)? Needs a short ideation pass before any code. |
