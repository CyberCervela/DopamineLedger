...
| ~~**Recurring quests**~~ | ✅ Done Session 36. Daily / weekly / monthly. Calendar-anchored (Monday for weekly). Silent expiry on skip — stale instance hard-deleted, fresh one created on next app open. Done path spawns next-period instance with `availableAt` gate to prevent double-dipping. `RecurringCadence` enum in `Quest.swift`. Badge on `QuestRow`. `sweepExpiredRecurringQuests()` in `ActivityListView.task` + `.onChange(scenePhase)`. |
