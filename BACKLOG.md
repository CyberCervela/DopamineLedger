...
| DL-20 | ARCHITECTURE | SessionEventSource abstraction — single validated entry point for all session triggers |
| DL-21 | FEATURE | HealthKit auto-sessions — detect completed workouts, offer to log as charger sessions |
...
#### DL-20 · SessionEventSource abstraction
Pure refactor: decouple session triggering from the UI so every input source (UI tap, Siri, HealthKit, future Watch or BLE device) feeds the same validated engine through one interface. Define a `SessionEventSource` protocol (source identity + intent: startCharge / startSpend / stop, activity reference, optional timestamp + external event ID for dedup). Single `SessionCoordinator` entry point validates EVERY intent against economy rules (debt blocking, concurrent session rules) regardless of source. Rejected intents return a typed reason, not a silent no-op. Refactor existing UI and Siri paths through it; behaviour must be byte-for-byte identical. **This is the integration hook DL-21 (HealthKit), DL-17 (Watch), and future BLE hardware all depend on — cheap to build now, expensive to retrofit later.** No SwiftData schema change, no new features.

#### DL-21 · HealthKit auto-sessions
Detect completed workouts via `HKWorkout` samples (read-only — no writing to HealthKit). On detected workout: local notification "Log 45 min of Running as a charge?" → user confirms → intent flows through `SessionCoordinator` (requires DL-20). Mapping UI in Settings: workout type → charger activity (e.g. Running → "Working out"); unmapped types are ignored. Works with app in background (`HKObserverQuery` + background delivery). Privacy policy and App Store privacy labels need review before shipping. **Three open design questions for PM before implementation:** (1) Always confirm, or auto-log silently? (Recommend: always confirm — user remains the conscious actor, consistent with `PHILOSOPHY.md`.) (2) Retroactive backfill on first enable, or forward-only? (Recommend: forward-only — backfill inflates balances and invites gaming.) (3) Use HealthKit duration as session duration, even if app was never open? (Recommend: yes, capped at 4h/event.) **Depends on:** DL-20.
...
| DL-18 | FEATURE | Motion System & micro-interactions — spring physics, haptics, animated numbers (neumorphic) |
| DL-19 | FEATURE | "Liquid" theme — iOS 26 Liquid Glass surfaces (requires DL-18 + Xcode 26 SDK) |
...
#### DL-18 · Motion System & micro-interactions (neumorphic)
Purely presentational — no new features, no data model changes. **Phase 1 (this item, ready for Claude Code):** central `MotionSystem.swift` defining all named animation tokens (zero inline `.animation(...)` durations may remain after — grep check); custom `DLButtonStyle` with haptics and the neumorphic press treatment (shadow from raised/extruded to inset/concave while pressed + slight scale); `.contentTransition(.numericText())` on all credit/balance displays; spring-animated list insert/delete/reorder. If neumorphic shadow styling is duplicated across views, consolidate into a single `NeumorphicSurface` modifier first and drive the pressed state through it. **Phase 2 (separate PR, PM review of Phase 1 first):** `matchedGeometryEffect` card→edit-sheet morph. **Phase 3 (separate PR):** hero moments (session complete, debt repaid) via `PhaseAnimator`. `MotionSystem` must be theme-aware in structure so a future theme can substitute its own motion language without touching views. Respects Reduce Motion (falls back to opacity fades). No regressions in timers, Live Activities, notifications.

#### DL-19 · "Liquid" theme (iOS 26 Liquid Glass)
Replace the classic iOS chrome with native iOS 26 `glassEffect` / `GlassEffectContainer` / `glassEffectID` APIs: translucent, depth-aware surfaces with fluid morphing. **Phase A:** ThemeProvider plumbing + glass surfaces on the main dashboard only → PM reviews on device. **Phase B (after PM review):** roll out to remaining screens; glass morphing on the start/stop control via `glassEffectID`. **Locked constraints:** deployment target stays iOS 17; all iOS 26 API behind `#available`; no glass-on-glass stacking anywhere; Reduce Transparency produces a usable legible fallback; stats/history text sits on solid backgrounds; `DLButtonStyle` resolves per-theme (neumorphic = shadow inset, liquid = `.glassEffect(.regular.interactive())`). **Requires:** DL-18 Phase 1 (MotionSystem + DLButtonStyle must exist and be theme-aware) + confirmed Xcode 26 build. **Open PM question before Phase B:** Does "Liquid" eventually replace the classic theme or live alongside it? (PM lean: replace — maintaining 3–4 motion languages is real ongoing cost for a solo project.)
...
| DL-22 | RESEARCH | Screen Time enforcement (FamilyControls) — feasibility memo before any implementation | memo (inline below) |
...
**DL-22 — Screen Time enforcement (FamilyControls):** Research-only — no implementation until the memo is written and PM approves. Output: written feasibility memo + recommendation (ship self-control mode / defer / kill). Research questions: (1) entitlement process and timeline for `com.apple.developer.family-controls` — approval criteria, rejection risk; (2) how `ManagedSettings` shields + `DeviceActivity` monitoring would map to DL ("shield selected apps unless a spender session is active") and constraints of the `DeviceActivityMonitor` extension (strict memory limits, no UI); (3) UX of failure: what happens when a session ends mid-stream, how escapable should the shield be (PHILOSOPHY question: commitment device vs. self-surveillance); (4) privacy posture: `FamilyControls` app tokens are opaque by design — verify this keeps the "no data collection" posture intact; (5) KIDS MODE scope check only: what changes if a parent controls a child's device (Family Sharing auth flow, App Store Kids Category rules, parental consent / COPPA-class obligations) — treated as a SEPARATE potential product, not a v1.x feature. **Note:** Path C in the "cross-app automation" research note is already flagged ⛔ BLOCKED due to iOS 26 API regressions — this item asks whether the FamilyControls architecture makes sense at all before the API block lifts.
...
| **Privacy policy — translate to all 6 non-English locales** | `docs/privacy.html` is English-only. The app ships in FR/DE/ES/JA/KO/ZH-Hans. Options: (1) one HTML file per locale (`privacy.fr.html` etc.) with a language-switcher link at the top, (2) a single page with a `<select>` toggle that swaps the visible language block via JS, (3) separate GitHub Pages routes per locale (`/privacy/fr/`, etc.). The App Store listing links to one URL — option 2 (single URL, JS toggle) is simplest to maintain. Design-first: agree on approach before writing any HTML. Low urgency — the English version is legally valid for all locales. |
...
| HealthKit workout observer (Path B → **DL-21**) | Promoted to **DL-21** (Tier 1) — see full spec in the Tier 1 section above. |
