...
|---|---|---|---|
| **Live Activity shows "0.0 cr spent" for spender sessions** | Screenshot 2: Dynamic Island shows "Social - T… / 0.0 cr spent" and never updates. Screenshot 3 confirms charger sessions work correctly (Reading → 129.9 cr earned). | `LiveActivityService.update()` likely passes `creditsMoved` which is negative for spenders. The Live Activity widget or the value passed to it may be treating a negative as zero (e.g. `max(0, creditsMoved)` somewhere, or displaying `abs()` before the value is actually non-zero). Investigate the spender branch in `LiveActivityService.update()` and the widget's `SessionLiveActivity` rendering logic. | Confirmed on device. Charger path works; spender path broken. |
...
| **Tab bar — content scrolls behind it** | The bottom navigation bar (Home / Stats / History) hard-clips the list: the last row stops flush at the bar edge. The filter pill bar at the top already handles this correctly — list content scrolls behind it with the bar maintaining its own background. The tab bar should do the same: extend the scroll view's content under the bar using `.safeAreaInset(edge: .bottom)` (or equivalent bottom padding on the `ScrollView`), and give `NeuTabBar` a solid `theme.colors.background` backing so the content is visible beneath it but doesn't bleed through. No blur needed — solid surface is consistent with the neumorphic language and matches the filter bar treatment. |
...
| 3 | bug | **Live Activity shows 0.0 cr spent for spenders** | Credit counter stuck at 0.0 for spender sessions; charger sessions work correctly. Investigate `LiveActivityService.update()` spender branch — likely a sign issue where negative `creditsMoved` is passed as zero to the widget. |
| 4 | bug | **Burn-down rate wrong during active session** | Balance 1 912 cr at 10 cr/min should show ~3h 11m; shows ~31h 52m (= balance ÷ 1 cr/min). Either the active-session rate is not fed to the burn-down calculation, or the burn-down intentionally hides on the active row and a global fallback uses a wrong default rate. Investigate before code. |
| 5 | feature | **Auto-pause sessions after 6 hours** | See Post-MVP features entry for full spec. UNTimeIntervalNotificationTrigger at 21 600s; two notification actions wired to existing ResumeSessionIntent / StopSessionIntent; countdown resets on manual pause/resume. |
| 6 | feature | **Activity list sort — most used / last used** | See Post-MVP features entry. Sort control in list header; creation order stays default; preference persisted via @AppStorage. Resolve design question first: interaction with existing filter tabs. |
| 7 | tech debt | **Integration tests for ledger-mutating flows** | SessionFinalizer.finalize and the repay flows in DebtView/ActivityMenuView have zero test coverage despite being the highest-consequence mutations. Add tests with an in-memory ModelContainer. Additive, zero UI risk. |
