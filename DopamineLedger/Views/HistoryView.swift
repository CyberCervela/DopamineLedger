...
struct HistoryView: View {
    ...
    // The stores are small and rarely change so this is fine for MVP.
    ...
    private enum HistoryEntry {
        case session(Session, Activity?)   // Activity? — nil if the activity was deleted
        case quest(Quest)

        // The date used for ordering and day-grouping.
        var date: Date {
            switch self {
            case .session(let s, _): return s.endedAt ?? s.startedAt
            case .quest(let q):      return q.completedAt ?? .distantPast
            }
        }
    }

    // Merged, sorted (newest first), and grouped by calendar day.
    private var dayGroups: [(header: String, entries: [HistoryEntry])] {
        ...
        let sessionEntries: [HistoryEntry] = allSessions
        ...
        // Group by the start-of-day for each entry's date.
    ...
    // "TODAY", "YESTERDAY", or "MON 26 MAY" — all caps, no localisation key needed
    // because DateFormatter respects the device locale automatically.
    private func dayHeader(for dayStart: Date, calendar: Calendar) -> String {
        ...
        if calendar.isDateInYesterday(dayStart) { return "YESTERDAY" } // TODO: localise if needed
        let f = DateFormatter()
...
private struct SessionHistoryRow: View {
    ...
    @Environment(\.languageBundle) private var lBundle
    let session:  Session
    let activity: Activity?   // nil if the originating activity was deleted
    ...
    private var subtitleText: String {
        let time = formatTime(session.startedAt)
        let duration = formatDuration(session.elapsed)
